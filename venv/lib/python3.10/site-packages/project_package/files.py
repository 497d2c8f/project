...
def write_keys_file():
	...
	filepath = os.path.join(CWD, f'keys__{sign_pk.hex()[:SUFFIX_LENGTH]}')
	write_file(
		filepath,
		**keys
	)
	print(f'\nФайл с ключами "{CWD}/keys__{sign_pk.hex()[:SUFFIX_LENGTH]}" успешно создан.')
	return keys
...
def get_voting_filepath(voting):
	voting_filepath = os.path.join(VOTINGS_WD, voting.get_v_id().hex()[:SUFFIX_LENGTH], f'voting__{voting.get_v_id().hex()[:SUFFIX_LENGTH]}')
	return voting_filepath


def get_keys_public_filepath(keys_file_data):
	keys_public_filepath = os.path.join(CWD, f'keys_public__{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}')
