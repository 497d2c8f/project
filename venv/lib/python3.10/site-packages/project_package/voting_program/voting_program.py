...
try:
	...
	def get_l_vp_sign_pk():
		...
		if input('\nВ голосовании может участвовать любой желающий?\n\n1. Да\n2. Нет\n\nВыберите и нажмите Enter: ') == '2':
			...
			keys_public_filepaths = get_filepaths('keys_public', directory=keys_public_files_dir, full_path=True)
...
#################################################''')
	...
	print('\nДля выхода из программы в любой момент можно нажать сочетание клавиш Ctrl + C.')
	...
	if option_1 == ELECTOR_MODE:
		...
		voting_filepath = format_path(input('\nПеретащите сюда файл голосования "voting" и нажмите Enter: '))
		...
		e_data_filepath = os.path.join(VOTINGS_WD, e_data['v'].get_v_id().hex()[:SUFFIX_LENGTH], f'e_data__{e_data["v"].get_v_id().hex()[:SUFFIX_LENGTH]}_{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}')
		msg_em_filepath = os.path.join(VOTINGS_WD, e_data['v'].get_v_id().hex()[:SUFFIX_LENGTH], f'msg_em__{e_data["v"].get_v_id().hex()[:SUFFIX_LENGTH]}_{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}_{e_data["m_sign_pk"].hex()[:SUFFIX_LENGTH]}')
	...
	if option_1 == MEDIATOR_MODE:
		...
		if option_2 == '1':
			...
			voting_filepath = format_path(input('\nПеретащите сюда файл голосования "voting" и нажмите Enter: '))
			voting = read_file(voting_filepath)['voting']
			msg_wm_1_filepath = format_path(input('\nПеретащите сюда файл "msg_wm_1" и нажмите Enter: '))
	...
	#
			m_data_filepath = os.path.join(VOTINGS_WD, m_data['v'].get_v_id().hex()[:SUFFIX_LENGTH], f'm_data__{m_data["v"].get_v_id().hex()[:SUFFIX_LENGTH]}_{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}')
			msg_mw_filepath = os.path.join(VOTINGS_WD, m_data['v'].get_v_id().hex()[:SUFFIX_LENGTH], f'msg_mw__{m_data["v"].get_v_id().hex()[:SUFFIX_LENGTH]}_{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}_{bytes(32).hex()[:SUFFIX_LENGTH]}')
		...
		if option_2 == '2':
			...
			msg_wm_2_filepath = format_path(input('\nПеретащите сюда с файл "msg_wm_2" и нажмите Enter: '))
			...
			write_file(m_data_filepath, main_key, **m_data)
			msg_ma_filepath = os.path.join(VOTINGS_WD, voting_dirname, f'msg_ma__{m_data["v"].get_v_id().hex()[:SUFFIX_LENGTH]}_{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}_{m_data["a_sign_pk"].hex()[:SUFFIX_LENGTH]}')
	...
	if option_1 == AGENCY_MODE:
		...
		if option_2 == '2':
			...
			voting_filepath = format_path(input('\nПеретащите сюда скачанный с сайта файл "voting" и нажмите Enter: '))
			voting = read_file(voting_filepath)['voting']
			msg_wa_filepath = format_path(input('\nПеретащите сюда файл с сообщениями посредников "msg_wa" и нажмите Enter: '))
			...
			a_data_filepath = os.path.join(VOTINGS_WD, a_data['v'].get_v_id().hex()[:SUFFIX_LENGTH], f'a_data__{a_data["v"].get_v_id().hex()[:SUFFIX_LENGTH]}_{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}')
			msg_voting_summary_filepath = os.path.join(VOTINGS_WD, a_data['v'].get_v_id().hex()[:SUFFIX_LENGTH], f'msg_voting_summary__{a_data["v"].get_v_id().hex()[:SUFFIX_LENGTH]}_{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}_{bytes(32).hex()[:SUFFIX_LENGTH]}')
	...
	if option_1 == READ_MSG_VOTING_SUMMARY_FILE:
		...
		s_a_vres_filepath = input('\nПеретащите сюда файл "s_a_vres" и нажмите Enter: ').replace('\'', '').strip()
	...
	if option_1 == CREATE_ONE_TIME_KEY_WITH_SPECIAL_CODE:
		...
		while True:
			input_string = input('\nСкопируйте сюда код для создания одноразового ключа и нажмите Enter: ')
