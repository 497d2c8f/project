...
#
		...
		msg_mw_filepath = os.path.join(VOTINGS_WD, m_data['v'].get_v_id().hex()[:SUFFIX_LENGTH], f'msg_mw__{m_data["v"].get_v_id().hex()[:SUFFIX_LENGTH]}_{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}_{bytes(8).hex()}')
...
if option_1 == AGENCY_MODE:
	...
	if option_2 == '2':
		...
		msg_voting_summary_filepath = os.path.join(VOTINGS_WD, a_data['v'].get_v_id().hex()[:SUFFIX_LENGTH], f'msg_voting_summary__{a_data["v"].get_v_id().hex()[:SUFFIX_LENGTH]}_{keys_file_data["sign_pk"].hex()[:SUFFIX_LENGTH]}_{bytes(8).hex()}')
